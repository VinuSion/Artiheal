...
uploadRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const folderExists = await cloudinary.api.sub_folders('', { root_folder: userId });
      ...
      const folderContents = await cloudinary.api.resources({
        type: "upload",
        prefix: `${userId}/`, // Restrict to the user's folder.
      });
      ...
      const result: any = await new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          {
            folder: userId, // Save to the user's folder.
          },
          (error, result) => {
            if (result) {
              resolve(result);
      ...
      if (!user) {
        return res
          .status(404)
          .json({ message: "Usuario no se pudo encontrar." });
      }
